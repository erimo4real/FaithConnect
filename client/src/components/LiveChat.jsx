  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-3">
      ...
      <button
        ...
        className="w-14 h-14 bg-primary hover:bg-primary-dark text-white rounded-full shadow-xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 overflow-hidden"
        aria-label={isOpen ? 'Close chat' : 'Chat with Bethel Church'}
      >
        {isOpen ? (
          <FaTimes className="w-6 h-6" />
        ) : (
          <img src="/churchlogo.png" alt="" className="w-full h-full object-cover" />
        )}
      </button>
    </div>
  );
