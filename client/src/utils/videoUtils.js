...
export function getVideoInfo(url) {
  ...
  if (yt) return { platform: 'youtube', id: yt[1], embedUrl: `https://www.youtube.com/embed/${yt[1]}`, thumbnail: `https://img.youtube.com/vi/${yt[1]}/maxresdefault.jpg`, embeddable: true };

  if (url.includes('facebook.com') || url.includes('fb.watch')) {
    return { platform: 'facebook', id: null, embedUrl: `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false`, thumbnail: null, embeddable: true };
  }

  const vm = url.match(/vimeo\.com\/(\d+)/);
  if (vm) return { platform: 'vimeo', id: vm[1], embedUrl: `https://player.vimeo.com/video/${vm[1]}`, thumbnail: null, embeddable: true };

  const tt = url.match(/tiktok\.com\/@[\w.-]+\/video\/(\d+)/);
  if (tt) return { platform: 'tiktok', id: tt[1], embedUrl: `https://www.tiktok.com/embed/v2/${tt[1]}`, thumbnail: null, embeddable: false };

  const ig = url.match(/instagram\.com\/(?:p|reel)\/([^/?#]+)/);
  if (ig) return { platform: 'instagram', id: ig[1], embedUrl: `https://www.instagram.com/p/${ig[1]}/embed`, thumbnail: null, embeddable: false };

  const dm = url.match(/dailymotion\.com\/video\/([^_?#]+)/);
  if (dm) return { platform: 'dailymotion', id: dm[1], embedUrl: `https://www.dailymotion.com/embed/video/${dm[1]}`, thumbnail: null, embeddable: true };

  return { platform: 'generic', id: null, embedUrl: url, thumbnail: null, embeddable: false };
}
