...
export function getVideoInfo(url) {
  ...
  const fb = url.match(/(?:facebook\.com|fb\.watch)\/|facebook\.com\/.*\/videos\//);
  if (fb || url.includes('facebook.com') || url.includes('fb.watch')) {
    const embedUrl = `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false`;
    return { platform: 'facebook', id: null, embedUrl, thumbnail: null };
  }
