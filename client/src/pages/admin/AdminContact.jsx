...
export default function AdminContact() {
  ...
  const handleStatus = async (id, status) => { try { await adminUpdateContactMessage(id, { status }); toast.success('Status updated'); load(); } catch (err) { toast.error(err.message); } };
  const handleDelete = async (id) => { if (!confirm('Delete this message?')) return; try { await adminDeleteContactMessage(id); toast.success('Message deleted'); load(); } catch (err) { toast.error(err.message); } };
                      ...
                      <tr key={item.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        <td className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <select value={item.status || 'unread'} onChange={(e) => handleStatus(item.id, e.target.value)} className="border border-gray-200 dark:border-gray-600 rounded-lg text-xs p-1.5 outline-none focus:ring-2 focus:ring-primary">
