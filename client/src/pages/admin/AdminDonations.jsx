...
import { adminFetchDonations, adminUpdateDonation, adminDeleteDonation } from '../../services/api';
...
export default function AdminDonations() {
  ...
  const load = async () => {
    try { setItems(await adminFetchDonations()); }
    ...
    <AdminLayout title="Donations">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center gap-4">
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 shrink-0">{items.length} donations</p>
                  ...
                  <tbody>
                    {paged.map((item) => (
                      <tr key={item.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        ...
                        <td className="py-3 pr-4 text-sm font-semibold text-gray-800 dark:text-gray-100">${Number(item.amount).toFixed(2)}</td>
                        <td className="py-3 pr-4"><span className="text-xs capitalize px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{item.cause}</span></td>
                    ...
                    {filtered.length === 0 && <tr><td colSpan="6" className="py-8 text-center text-gray-400 dark:text-gray-500 text-sm">{items.length === 0 ? 'No donations yet' : 'No matching results'}</td></tr>}
                  </tbody>
                </table>
              </div>
              {totalPages > 1 && (
        ...
        {selected && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 h-fit">
            ...
            <div className="space-y-3">
              <div><p className="text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">Amount</p><p className="text-2xl font-bold text-primary">${Number(selected.amount).toFixed(2)}</p></div>
